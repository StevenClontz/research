...
\begin{lemma}
    Let \(X\) be a Hausdorff space of cardinality less than
    \(\aleph_\omega\) which is locally countable, locally compact, and the
    closure of each set has the same cardinality as the set.
  ...
  \begin{proof}
    ...
    copies of \(X\).
    So suppose \(n>0\) and
    that \(X=\{\alpha:\alpha<\omega_n\}\) has such a topology.
    For each \(\alpha<\omega_n\), \(\gamma_\alpha\) may be chosen such that
    both the closure of \(\alpha\) in \(X\) and a countable neighborhood of
    \(\alpha\) are subsets of \(\gamma_\alpha\). Note that
    the set
    \(C'=\{\lambda<\omega_n:\alpha<\lambda\Rightarrow\gamma_\alpha<\lambda\}\)
    is a cub subset of \(\omega_n\).
    It will be convenient later to use the cub \(C=C'\cap L\), where
    \(L\) contains the limits of limit ordinals in \(\omega_n\).
    ...
    for some \(\alpha<\lambda\); thus \(U\setminus\cl\alpha\)
    is a neighborhood of \(\beta\) missing \(\lambda\), showing that \(\lambda\)
    is clopen.

    Let \(\tilde X=\omega_n\times 2\).
    By induction on \(\lambda \in C\) we will define a topology
    for \(\omega_n\times\{0\}\cup \lambda\times \{1\}\) such that
    \begin{itemize}
      \item \(\omega_n \times \{0\}\) is an open copy of \(X\),
      \item the space is locally compact, and
      \item for each \(\alpha<\lambda\), the closure of \(\alpha\times 2\) is
        an \(\omega\)-bounded  subset of \(\lambda\times 2\).
    \end{itemize}

    In the case that \(n=1\), if \(\lambda\) is a limit in \(C\),
    then we simply give
    \(
      \omega_n\times\{0\}\cup\lambda\times\{1\}
    \) the topology induced by the union of the previously defined
    \(
      \omega_n\times\{0\}\cup\lambda'\times\{1\}
    \).
    Otherwise we choose an increasing sequence of limit
    ordinals \(\{\alpha_k : k\in \omega\}\) with limit \(\lambda\)
    such that \(\alpha_0\) is the predecessor of
    \(\lambda\) in \(C\) (or \(0\) if \(\lambda\) is the least element of
    \(C\)).
    ...
    So we may choose sets \(U_k\) for \(k<\omega\) which are
    ...
    \(
      ...
      U_n
        \subseteq
      \lambda\times\{0\}\cup\alpha_0\times\{1\}
    \).

    We need only describe a base for the points in
    \((\lambda\setminus\alpha_0)\times\{1\}\); we do so by letting
    \(\<\alpha,1\>\) be isolated when \(\alpha\not\in\{\alpha_k:k<\omega\}\),
    and letting \(\<\alpha_k,1\>\) be a compactifying point for
    \(U_n\cup(\alpha_k\times\{1\})\).

    This results in a locally compact space: for \(\alpha\geq\lambda\),
    \(\<\alpha,0\>\) has its usual compact neighborhoods within the copy
    \(\omega_n\times\{0\}\) of \(X\).
  ...
  \begin{proof}
    Take a topological sum of \(\aleph_{n+1}\) copies of \(\omega_n\)
    with an appropriate topology (a convergent sequence for \(\omega\), the
    result of this theorem for larger cardinals),
    and apply the previous lemma.

    TODO: write an argument for the note.
  \end{proof}
