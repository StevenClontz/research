...
\section{Introduction}
  ...
  \begin{proposition}
    Let \(X\) be a \(T_2\) locally countable \(\omega\)-bounded space.
    Then for each \(A\subseteq X\), \(\cl A\) has the same cardinality and
    weight as \(A\).
  \end{proposition}
  ...
  \begin{lemma}
    ...
    \(\aleph_\omega\) which is locally countable, locally compact,
    and the closure of each set has the same cardinality as the set.
  ...
  \begin{proof}
    ...
    that \(X=\omega_n\) has such a topology. Note that
    \(X\) has a base of compact open neighborhoods
    with countable cardinality. To see this,
    ...
    is a cub subset of \(\omega_n\) with a cub subset
    \(C\) containing only limits of limit ordinals.
    ...
    \begin{itemize}
      ...
      \item \(\lambda\times 2\) is open, and also closed when \(\cf\lambda>\omega\),
      \item the space has a base of compact open neighborhoods with
    ...
    a compact open neighborhood of countable cardinality
    for points in \(\lambda\times 2\), and for points in
    \((\omega_n\setminus\cl\lambda)\times\{0\}\) we may use a compact
    open neighborhood of countable cardinality from \(X\setminus\lambda\).
    Finally let \(\<\alpha,0\>\in(\cl\lambda\setminus\lambda)\times\{0\}\)
    and let \(K\subseteq\cl\lambda\times\{0\}\cup\alpha_0\times\{1\}\) be a
    compact open neighborhood in that subspace missing \(\alpha_0\times 2\).
    Then for each \(k<\omega\), \(\<\alpha_k,1\>\) has a neighborhood missing
    the compact set \(K\cap U_k\), so \(K\) gains no new limit points in
    ...
    is easily seen to be locally compact and locally countable. It is also
    \(\omega\)-bounded as any countable set is contained in some
    \(\alpha\times 2\) for \(\alpha<\lambda\in C\). Finally by the previous
    proposition, the closure operation cannot increase cardinalities.
  \end{proof}
