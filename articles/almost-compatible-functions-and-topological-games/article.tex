...
\section{Introduction}
  ...
  \begin{lemma}[\cite{MR597342}]
    ...
    for $A\subseteq\omega$ (abusing notation and letting \(n\) represent
    its own $\operatorname{Fn}(\omega_2,2)$-name and
    ...
    and the valuation of $\dot A(G_\mu)$ by the tail
 ...
 \begin{lemma}[\cite{MR597342}]
  ...
  \begin{proof}
    ...
    names such that \(f_{A}\) is a one-to-one function from \(A\) into
    ...
    Let \(\lambda = M\cap \omega_2=\sup(M\cap\omega_2)\).
    Consider the name
    \[
      ...
      \{
        ( (\lambda+k,m), p)
      :
        p\in \mathop{Fn}(\omega_2,2);k,m\in \omega
      \}
    \]
