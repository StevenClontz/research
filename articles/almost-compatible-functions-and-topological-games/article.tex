...
\section{Introduction}
  ...
  in the affirmative as \(\alcompS{\mf c^+}\) fails, but we will show that assuming
  ...
  \begin{definition}
    The statement \(\alcompSP{\kappa}\) weakens \(\alcompS{\kappa}\) be only requiring
  ...
  \begin{theorem}
    \(\alcompSP{\kappa}\) holds whenever there exists a cofinal Kurepa family on \(\kappa\).
  \end{theorem}
  ...
  was previously shown in \cite{MR539228}.
  ...
  We note that in
  \cite{MR539228} a \(T_2\) locally countable \(\omega\)-bounded space
  was called \term{splendid}, and the claim that there exists a splendid
  space of cardinality \(\kappa\) was called \(S(\kappa)\).
  ...
  \begin{theorem}
    ...
    \(\omega\)-bounded topology on \(\omega_k\). (That is, \(S(\omega_k)\)
    for all \(k<\omega\)).
  \end{theorem}
  ...
  \begin{lemma}
    ...
    topological space is a Kurepa family cofinal in \([X]^\omega\).
  \end{lemma}
  ...
  The above lemma may alternatively be stated as ``\(S(\kappa)\) implies
      the existence of a cofinal Kurepa family on \(\kappa\)''.
  ...
  This corollary is also an observation of Todorcevic
  ...
  With this we have our desired result.

  \begin{corollary}
    \(\alcompSP{\aleph_k}\) holds for all \(k<\omega\). Under \(CH\), we have
    both \(\alcompSP{\aleph_2}\) and \(\neg\alcompS{\aleph_2}\).
  \end{corollary}
  ...
  \begin{corollary}[\(V=L\)]
    \(\alcompSP{\theta}\) holds for all cardinals.
  \end{corollary}

  In particular, \(\alcompS{\omega_2}\) fails under \(CH\), showing the two are
  distinct. Actually, \(CH\) is not required to have \(\alcompS{\omega_2}\) fail.
  ...
  On the other hand, it's also consistent that \(\alcompSP{\theta}\) can fail.

  \begin{theorem}
    There's a model where \(\alcompSP{\omega_\omega}\)
    fails.
  \end{theorem}
  ...
  \begin{question}
    Is \(\alcompSP{\theta}\) equivalent to having a Kurepa family on \(\theta\)?
  \end{question}
...
\end{figure}
  ...
  \begin{corollary}
    It is consistent that \(\alcompSP{\omega_\omega}\) fails,
    but as \(\alcompSP{\omega_k}\) holds for all \(k<\omega\), we have
  ...
  \(\alcompSP{\kappa}\) and thus \(\pl F\kmarkwin2\schFillIntGame{\kappa}\)
  always holds.
