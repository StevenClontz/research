...
\section{Introduction}
  ...
  % TODO: read through Alan's proofs and fit to rest of document
...
% \end{figure}
  ...
  \end{proof}
    ...
    %TODO maybe this can be made to work...
    % In \cite{MR1129143} Scheepers used a proof of Todorcevic to demonstrate
    % \(\alcompS\kappa\Rightarrow\pl F \ktactwin2\schFillStrictGame\kappa\).
    % For the convenience of the reader we explicitly demonstrate
    % the generalization as follows.

    % Let \(\{f_A:A\in[\kappa]^{\leq\aleph_0}\}\) witness
    % \(\alcompSP\kappa\), and define \(g_A:A\to\omega\) by
    ...
    % \(\alpha\in C_n\). Of course if
    % \(f_{C_{n+m}}(\alpha)\not=f_{C_{n+m+1}}(\alpha)\) for some
    % \(m<\omega\) we have \(\alpha\in\sigma(\<C_{n+m},C_{n+m+1}\>)\) immediately.
    % Otherwise let \(f_{C_{n+m}}(\alpha)=f_{C_{n+m+1}}(\alpha)=N\)
    % for all \(m<\omega\).

  It was left open if \(\alcompSP\kappa\) was equivalent to these
  game-theoretic conditions. The answer is consistently negative.
  ...
  \begin{corollary}
    ...
    but as \(\alcompSP{\aleph_k}\) holds for all \(k<\omega\), we have
    \(\pl F\kmarkwin2\schFillIntGame{\omega_\omega}\) and
    \(\pl F\kmarkwin2\schFillInitialGame{\omega_\omega}\).
  \end{corollary}

  Our topological question \ref{mainQuestion} remains unsolved; however,
