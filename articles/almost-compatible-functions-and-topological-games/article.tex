...
\section{Introduction}
  ...
  We take the following lemma from Kunen.

 \begin{lemma}[\cite{MR597342}]
    Assume that \(G\subset \operatorname{Fn}(\omega_2,2)\) is a generic filter
    over a model \(M\),
    and let \(\mu\in \omega_2\). Then the generic extension \(M[G]\) can be
    regarded as forcing with
    \(\operatorname{Fn}(\omega_2\setminus \mu, 2)\) over the model \(M[G_\mu]\).
    Let \(\dot n,\ddot n\) be a
    \(\operatorname{Fn}(\omega_2,2)\)-,
    \(\operatorname{Fn}(\omega_2\setminus\mu,2)\)-name respectively for each
    \(n<\omega\). Then for each \(\operatorname{Fn}(\omega_2,2)\)-name \(\dot A\)
    for \(A\subseteq\omega\), there is a canonical
    \(\operatorname{Fn}(\omega_2\setminus \mu, 2)\)-name
    \[
      \ddot A
        =
      \{
        (\ddot n,p\rest [\mu,\omega_2))
      :
        (\dot n,p)\in \dot A
      \}
    \]
    ...
    \(
      A
        =
      \operatorname{val}(\dot A,G)
        =
      \operatorname{val}(\ddot A,G_\mu)
    \).
  \end{lemma}
  ...
  \begin{proof}
    The forcing poset is \(\mb P=\operatorname{Fn}(\omega_2, 2)\).
    Let \(\{ \dot f_{A} : A\in [\omega_2]^\omega\}\) be a family of
    \(\mb P\)-names such that \(f_{A}\) is a one-to-one function from \(A\)
    into \(\omega\) in the ground model. Let \(M\) be an elementary submodel
    of \(ZFC+CH\) containing all its countable subsets, containing \(\mb P\),
    and satisfying \(|M|=\omega_1 =\mathfrak c\).

    Let \(\lambda\) be the ordinal \(M\cap \omega_2\). For
    \(\alpha\leq\lambda\) we may assume
    \(
      \dot f_{[\alpha,\alpha+\omega)}
        =
      \{
        ((\alpha+k_p,m_p)^\cdot, p)
      :
        p\in C_\alpha
      \}
    \)
    for some countable \(C_\alpha\subseteq\operatorname{Fn}(\omega_2,\omega)\).
    Let
    \(
      \operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
        =
      \bigcup_{p\in C_\alpha}\operatorname{dom} p
    \).

    For each \(\alpha\leq\lambda\),
    let \(\delta_{\alpha}\) denote the order type of
    \(\operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})\setminus \alpha\),
    and when \(\mu\leq\alpha\) and
    \(
      \operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
        \cap
      \alpha
      ...
      \mu
    \)
    let \(\varphi'_{\mu,\alpha}\) be the order-preserving bijection from
    \(
      \mu\cup
      \left(\operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
        \setminus
      \alpha\right)
    \)
    onto the ordinal \(\mu+\delta_\alpha\).
    This lifts canonically to an order-preserving bijection
    \(
      \varphi_{\mu,\alpha}
        :
      \operatorname{Fn}(\mu\cup
        \left(\operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
          \setminus
        \alpha\right)
      ,2)
        \to
      \operatorname{Fn}(\mu+\delta_\alpha,2)
    \).

    We now fix \(\mu<\lambda\) such that
    \[
      \bigcup_{\alpha\leq\lambda}
      \operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
        \cap
      \alpha
        \subseteq
      \mu
    \]
    (TODO how?) and define the \(\mb P\)-name
    \[
      F
      %   =
      % \varphi_{\mu,\lambda}(\dot f_{[\lambda,\lambda+\omega)})
        =
      \{
        ((\lambda+k_p,m_p)^\cdot,\varphi_{\mu,\lambda}(p))
      :
        ((\lambda+k_p,m_p)^\cdot,p)\in\dot f_{[\lambda,\lambda+\omega)}
      \}
    .\]

    So in \(M\), we may reason (TODO why)
    that for each \(\beta<\omega_2\) there exists
    some \(\alpha\in\omega_2\setminus\beta\) where
    \[
      \operatorname{supp}(\dot f_{[\alpha,\alpha+\omega)})
      \cap \alpha\subseteq \mu
    \]
    and
    \[
      F
        =
      \{
        ((\alpha+k_p,m_p)^\cdot,\varphi_{\mu,\alpha}(p))
      :
        ((\alpha+k_p,m_p)^\cdot,p)\in\dot f_{[\alpha,\alpha+\omega)}
      \}
    .\]

    But now, this means not only is there an \(\alpha<\lambda\) such that
    \( F = \varphi(\dot f_{[\alpha,\alpha+\omega)})\),
    but also that there is an increasing sequence
    \(\{\alpha_\xi : \xi  \in \omega_1\}\subseteq \lambda\)
    satisfying that \(F=\varphi(\dot f_{[\alpha_\xi,\alpha_\xi+\omega)})\) and
    \(\operatorname{supp}(\dot f_{[\alpha_\xi,\alpha_\xi+\omega)})\) is
    contained in \(\alpha_{\xi+1}\) (TODO why not \(\mu\)?).

    Choose such a sequence, and let
    \(A = \bigcup_{0<n<\omega} [\alpha_n,\alpha_n+\omega)\). Consider
    the name \(\dot f_A\) in \(M\). Note all the \(((\beta,m)^\cdot, p)\)
    appearing in \(\dot f_A\) have the property that
    \(\operatorname{dom}(p)\) is contained in \(M\) (QUESTION: isn't the
    domain of any member of \(\mb P\) in the domain?)
    There is, within \(M\), a name \(\dot g\) satisfying that
     \(\dot f_A(\alpha_n+k) = \dot f_{[\alpha_n,\alpha_n+\omega)}(\alpha_n+k)\)
    for all \(k >\dot g(n)\) (TODO: why?).

    (TODO catch up to here)

            We now apply the above Lemma and work in \(M[G_\mu]\).
            ...
            \varphi(q\restriction \operatorname{supp}(
            ...
            \[ q_3 = \varphi^{-1} \circ \varphi_{\mu,\alpha_n}
            ...
            there is a  \(\beta'\in\mathop{dom}(q_2)\) such that \(
             \varphi(\beta) = \varphi_{\mu,\alpha_n}(\beta')\),
...
% \end{figure}
  ...
  % \section{Generalized Kurepa families and almost-compactible functions}
