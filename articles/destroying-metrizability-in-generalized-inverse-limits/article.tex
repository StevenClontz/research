...
Let us say a subset $S$ of $[0,1]^2$ satisfies condition $\Gamma$ if, for some $x, y \in [0,1]$ with $x \ne y$, $\<x,x\> \in S$, $\<y,y\> \in S$, and either $\<x,y\>$ or $\<y,x\> \in S$. Let $\Delta = \{\<a,a\> \ | \ a \in [0,1]\}$, and notice that $[0,1]^2 \setminus \Delta$ is the union of two disjoint open sets.
...
\bibitem{varagona} Scott Varagona, {\it Generalized Inverse Limits Indexed by Totally Ordered Sets}, preprint (submitted to Topology Proceedings), 2015.
