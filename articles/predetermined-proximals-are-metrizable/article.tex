...
% This stuff wades into D-space theory; it's pretty unclear if 
% what I called a dual game is actually dual.
...
%However, it can be somewhat emulated by the following selection game.
%
%\begin{definition}
...
%\begin{itemize}
%\item For each \(m<\omega\), \(x_n\in U_m\) for cofinitely-many \(n<\omega\)
...
%%considering only paracompact spaces.MAYBE
%%\end{theorem}
%%\begin{proof}
%%Let \(\sigma\) be a winning predetermined strategy for \(P1\) in the proximal game
%%such that \(\sigma(n+1)\subseteq\frac{1}{2}\sigma(n)\) for all \(n<\omega\).
%%Then define the neighborhood assignment
...
%%and \(\bigcap\setB{\sigma(n)[x_n]}{n<\omega}\not=\emptyset\).
%%Pick some \(S(n)>n\) such that \(x_{m}\in \sigma(n)[x_n]\) for all
%%\(m\geq S(n)\).
%%It follows that \(x_{S^{n+1}(0)}\in\sigma(S^n(0))[x_{S(n)}]\subseteq\sigma(n)[x_{S(n)}]\),
%%so \(\alpha\) defined by \(\alpha(n)=x_{S^n(0)}\) is a legal counterattack to
%%\(\sigma\). Since \[\bigcap\setB{\sigma(n)[\alpha(n)]}{n<\omega}\supseteq
%%\bigcap\setB{\sigma(S^n(0))[x_{S^n(0)}]}{n<\omega}\]\[\supseteq
%%\bigcap\setB{\sigma(n)[x_n]}{n<\omega}\not=\emptyset\]
%%we have that \(\alpha\) must converge to some \(x\) 
%%since \(\sigma\) is a winning strategy for \(P1\).
%%So consider an open neighborhood \(U\) of \(x\); it contains \(\alpha(n)\) for
%%cofinite \(n<\omega\), say \(n\geq N\). Then consider the open neighborhood 
%%\(\frac{1}{2}\sigma(S^N(0))[x]\)
%%of \(x\), which contains \(\alpha(n)\) for cofinite \(n<\omega\); it follows
%%that for \(z\in .
%%\end{proof}
%
%\begin{definition}
%Let \(\mc{N}_X=\setB{N_x}{x\in X}\) where \(N_x=\setB{\tuple{x,U}}{x\in U\in\mc T_{X}}\).
...
%
%\begin{proposition}
%\(\ssg{\mc A_X}{\mc B}\) is dual to 
%\(\ssg{\mc N_X}{\neg\mc B}\) for any set \(\mc B\).
%\end{proposition}
%\begin{proof}
...
%\end{proof}
%
%It's unclear when the selectively proximal game is equivalent to the proximal
%game: there's no way to require \(x_{n+1}\in U_n\) in the selection
%game context, and while the neighborhood assignments could be forced to be uniform,
%there's no clear way to enforce this in the dual. 
%However, this easy selection game result provides the inspiration
%to define the following game.
%
%\begin{definition}
%The game \(DP(X)\) is played as follows. During round
%\(0\), \(P1\) chooses \(x_0\in X\), followed by \(P2\) choosing an open neighborhood
%\(U_0\) of \(x_0\). Then during round \(n+1\), \(P1\) chooses \(x_{n+1}\in U_n\),
%followed by \(P2\) choosing an open neighborhood \(U_{n+1}\) of \(x_{n+1}\). 
%Then \(P2\) wins
%if the \(x_n\) converge or \(\bigcap\setB{U_n}{n<\omega}=\emptyset\).
%\end{definition} 
%
%\begin{theorem}
%The proximal game and \(DP(X)\) are dual for any paracompact space \(X\).
%\end{theorem}
%\begin{proof}
%Let \(\sigma\) be a winning predetermined strategy for \(P1\) 
%in the proximal game.
%Then define the Markov strategy \(\tau\) for \(P2\) in \(DP(X)\)
%by \(\tau(x,n)=\sigma(n)[x]\). Let \(\alpha\) attack \(\tau\),
%so \(\alpha(n+1)\in\tau(\alpha(n),n)=\sigma(n)[\alpha(n)]\).
%Then \(\alpha\) also attacks \(\sigma\), which is winning,
%so either \(\alpha\) converges or 
%\(\bigcap\setB{\sigma(n)[\alpha(n)]}{n<\omega}=
%\bigcap\setB{\tau(\alpha(n),n)}{n<\omega}=\emptyset\). Thus
%\(\tau\) is winning for \(P2\).
%
%Let \(\sigma\) be a winning Markov strategy for \(P2\)
%in \(DP(X)\).
%\end{proof}
