...
\begin{defn}
  A \term{uniform space} is a pair $\<X,\mc{D}\>$ where $X$ is a set, and $\mc{D}$ is a uniformity. A \term{uniformity} is a filter (see e.g. \cite{k}) on subsets of $X^2$, called \term{entourages}, such that for each entourage $D\in \mc D$:
  \begin{itemize}
...
Since proximal spaces are closed under closed subsets and $\Sigma$-products \cite{b}, it follows (as noted by Nyikos) that
