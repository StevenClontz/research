...
\begin{definition}
  A space is \term{dense-separable} if every dense subspace is separable.
\end{definition}

As noted earlier, all \(SS\) spaces are separable as witnessed
by choosing \(D_n=X\); in fact, they
are dense-separable as witnessed by choosing \(D_n=D\) for
any dense subset \(D\) of \(X\).
As such, it is safe to assume that the first player
...
as shown below.
Note that the knowledge of all \(D_n\) is used to define \(D\)
in this proof, preventing a trivial modification to answer the
above question.

\begin{proposition}\label{ds-ss}
  Let \(X\) be a dense-separable space such that every countable dense
  subspace \(D\subseteq X\) is \(SS\). Then \(X\) is \(SS\).
\end{proposition}

\begin{proof}
  Take \(\{D_n:n<\omega\}\) with \(D_n\subseteq X\) countable dense in
  \(X\), and therefore countable dense in the \(SS\) subspace 
  \(D=\bigcup\{D_n:n<\omega\}\),
  So choose \(E_n\in[D_n]^{<\omega}\)
  with \(cl_D(\bigcup\{E_n:n<\omega\})=D\) and therefore 
  \(cl(\bigcup\{E_n:n<\omega\})\supseteq cl(D)=X\). Thus the \(E_n\)
  witness that \(X\) is \(SS\). 
\end{proof} 
...
The author also thanks the anonymous referee for their helpful suggestions
to improve Propositions \ref{cofinalSequence} and \ref{ds-ss}, and the observation
of Theorem \ref{m-om}.
