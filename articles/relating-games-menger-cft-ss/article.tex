...
\begin{proof}
  ...
  % Let \(\sigma_0(\<\mc U_0\>)=\{I_0\}\) where \(I_0\) is a co-countable set
  % containing \(\infty\). Denote
  % \(\onePtLind{\omega_1}\setminus I_0=\{\alpha_{0,n}:n<\omega\}\),
  % and then let \(\sigma_0(\<\mc U_0,\dots,\mc U_{n+1}\>)\) contain exactly
  % one superset of \(\{\infty\}\cup\{\alpha_{0,n}\}\).
...
% It's easily seen that \(CFT^+\) is a hereditary property for all subspaces.
