...
\subsection{Absolutely proximal}
  ...
  So all proximal compacts are absolutely proximal compacts.
...
\section{Limited information analogues}
  ...
  \begin{definition}
    ...
    is a strategy which considers only the \(k\) most recent
  ...
  \begin{definition}
    ...
    which considers only the round number and the \(k\)
    most recent moves of the opponent.
  ...
  The motivation for this paper follows from the following results
  characterizing compact proximal spaces as Corson compact spaces.
  ...
  \begin{definition}
    ...
    \[
      ...
      \right\}
    \]
    When \(0\in X=X_\alpha\)
    and \(z(\alpha)=0\) for all \(\alpha<\kappa\),
    then we write
    \(\SigmaPower{X}{\kappa}=\SigmaProd_{\alpha<\kappa}^z X_\alpha\).
  \end{definition}
  ...
  with Gary Gruenhage in \cite{MR3227201} affirmatively.
...
\subsection{\(\Sigma^\star\)-products and Eberlein compacts}
  ...
  \begin{definition}
    ...
    \]
    When \(0\in X\) and
    \(z(\alpha)=0\) for all \(\alpha<\kappa\),
    we write
    \(\SigmaStarPower{X}{\kappa}=\SigmaStarProd_{\kappa}^z X\).
  \end{definition}
  ...
  \begin{theorem}
    ...
    \(\pl D\markwin\bellConGame{\SigmaStarProd_{\kappa}^z X}\).
  \end{theorem}

  \begin{proof}
    For \(x\in\SigmaStarProd_{\kappa}^z X\), let
      \[
        \supp_n(x)
          =
        \left\{
    ...
    Define a strategy \(\tau\) for \(\pl D\) by
      \[
        \tau(\emptyset,0)
          =
        \left(\SigmaStarProd_{\kappa}^z X\right)^2
      \]
      \[
        \tau(\<p\>,n+1)
          =
    ...
    Let \(a:\omega\to \SigmaStarProd_{\kappa}^z X\) be a legal
...
\subsection{\(\sigma\)-products and strong Eberlein compacts}
  ...
  \begin{proof}
    ...
    Choose an arbitrary \(x_\emptyset\in X\) and \(\epsilon_0>0\),
    ...
    Assume \(X\) is not strong Eberlein compact. One of two things must hold.
      \begin{enumerate}
        ...
        \item
          ...
          \(\pl D\nottactwin \bellConGame{X}\).
      \end{enumerate}
    Thus \(X\) is not tactic-proximal. Corollary 2.29 gives the other
    direction.
  \end{proof}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item (as noted earlier)
        ...
        \]
    \end{enumerate}
    Finally, it follows from Lemmas 2.30 and 2.31
    that \(\pl D\nottactwin\bellConGame{X}\)
