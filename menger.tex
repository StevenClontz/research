...
\begin{document}
  ...
  \begin{proposition}
    $X$ is compact if and only if $S \tactwin \mengame{X}$
  \end{proposition}

  \begin{proof}
    Assume $X$ is compact. For each open cover played by $C$, pick the finite subcover.

    Assume $S$ has a winning tactical strategy. For any open cover, have $C$ play only it during the entire game. $S$'s only choice must be a finite subcover.
  \end{proof}

  \begin{proposition}
    If $X$ is $\sigma$-compact then $S \markwin \mengame{X}$
  \end{proposition}

  \begin{proof}
    Let $X=\bigcup_{n<\omega} X_n$ for compact $X_n$. On round $n$, $S$ picks the finite subcover of $C$'s open cover of $X_n$.
  \end{proof}

  \begin{question}
    Does $S \markwin \mengame{X}$ imply $X$ is $\sigma$-compact?
  \end{question}

  Let $\sigma(\mathcal{U},n)$ be $S$'s winning Markov strategy.
