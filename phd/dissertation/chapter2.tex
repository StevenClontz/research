...
$W$ is a Borel subset of $M^\omega$ \cite{martin}. It's an easy corollary that
...
\begin{defn}
  ...
  $\nu_k:M^{<\omega}\to M^{\leq k}$ defined by
    ...
    \]
  and the \term{$k$-Mark\"ov fog-of-war} is the function
...
\begin{defn}
  A \term{$k$-tactical strategy} or \term{$k$-tactic} is a limited information
  strategy of the form $\sigma\circ\nu_k$.

  A \term{$k$-Mark\"ov strategy} or \term{$k$-mark} is a limited information
  strategy of the form $\sigma\circ\mu_k$.
\end{defn}
...
example of a Baire space which is not weakly $\alpha$-favorable, and
