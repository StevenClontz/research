...
\begin{lem}
  If $\pl K\ktactwin{k+2}\gruKPGame{\pmb X}$, then
  $\pl K\ktactwin{k+1}\gruKPGame{\pmb X}$.
\end{lem}

\begin{proof}
  \newcommand{\hi}{\text{height}}
  \newcommand{\de}{\text{depth}}
  Fix an arbitrary $y\in X$. For $\pmb t\in \pmb X^{<\omega}$, define the
  following:
    \begin{itemize}
      \item $\hi(\pmb t)<\omega$ satisfying
            $|s|\leq \hi(\pmb t)$ for each
            $\<x,s\>\in(X\times 2^{<\omega})\cap \ran{\pmb t}$
      \item $\de(\pmb t)\in [X]^{<\omega}$ satisfying
            $x\in\de(\pmb t)$ for each
            $\<x,s\>\in(X\times 2^{<\omega})\cap \ran{\pmb t}$
      % \item $\pmb R(\pmb t)\in [\pmb X^{<\omega}]^{<\omega}$ such that
      %       $\pmb r\in \pmb R(\pmb t)$
      %       satisfies one of the following:
      %   \begin{itemize}
      %     \item $\pmb r(i)=c$ where $c\in C\cap\ran{\pmb t}$
      %     \item $\pmb r(i)=\<x,s\>$ where $s\leq\hi(\pmb t)$,
      %                                     $x\in\de(\pmb t)$
      %     \item $\pmb r(i)=\<y,s\>$ where $s\leq\hi(\pmb t)$
      %   \end{itemize}
    \end{itemize}

  For each compact set $C$, define $F(C)\in[\omega_1]^{<\omega}$ and
  $m(C)<\omega$ such that $C\subseteq K'_{F(C)}\cup L_{m(C)}$.

  Let $\sigma$ be a winning $k+2$-tactic for $\pl K$.
  For $\pmb t\in \pmb X^{<\omega}$, let $\pmb r\preceq \pmb t$ denote that
  $\pmb r$ is a subsequence of a permutation of $\pmb t$, and let
  $\pmb r\preceq_i \pmb t$ denote that $\pmb r$ is a subsequence of a
  permutation of $\pmb t\concat\<\<y,s\>\>$ for some $s\in 2^{\leq i}$.

  Let $M(\pmb t)=\max_{\pmb r\preceq \pmb t} m(\sigma(\pmb r\rest^{k+2}))$.
  We construct a $k+1$-tactic $\tau$ for $\pl K$ as follows:
    \[
      \tau(\pmb t)
        =
      \bigcup_{\pmb r\preceq_{|M(\pmb t)|} \pmb t}
      \sigma(\pmb r)
    \]
  Let $\pmb p:\omega\to\pmb X$ be a legal attack against $\tau$, so
    \[
      \pmb p(n)
        \not\in
      \bigcup_{m\leq n}
      \tau(\pmb p\rest^{k+1} m)
    \]
  Define the attack $\pmb q:\omega\to\pmb X$ as follows for $i<\omega$,
  $j<k+1$:
    \[
      \pmb q((k+2)i+j) = \pmb p((k+1)i+j)
    \]
    \[
      \pmb q((k+2)i+(k+1)) = \<y,a_{|M()|}\>
    \]
...
% \begin{prop}
% $K \win \gruKPGame{\pmb X}$.
% \end{prop}

% \begin{thm}(cor of G, game-theoretic proof by me)
% $K\not\tactwin\gruKPGame{\pmb X}$.
% \end{thm}

% \begin{thm}
% $K\not\ktactwin{k}\gruKPGame{\pmb X}$.
% \end{thm}
