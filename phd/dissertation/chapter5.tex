...
\begin{proof}
  Let $\sigma$ be a predetermined strategy for $\pl K$, and define the
  counterattack $H:\omega\to\mc K(X)$ by
...
\begin{proof}
  ...
  $\pl P$ against $\sigma$. For $n<\omega$, note that for any $f\in A$,
  $f$ maps only $n$ points into $n^2$, so the range of $f$ is coinfinite.
  Then $\mc I=\{\ran f : f\in A\}$ is an ideal, and if
  $\mc F\supseteq\{\omega\setminus I: I\in\mc I\}$, then $\sigma$ is a
