...
\begin{proof}
  ...
  % $r$ a subsequence of $s$ implies $\sigma(r,m)\subseteq\sigma(s,n)$.
  % Define $n:\mb X\to\omega$ by
  %   \[
  %     n(x)
  %       =
  %     \left\{
  ...
  %     \right.
  %   \]
  % and use $n$ to define the $k+1$-tactic $\tau$ by
  %   \[
  %     \tau(\emptyset)=\sigma(\emptyset,0)
  %   \]
  %   \[
  %     \tau(t\concat\<x\>)
  %       =
  ...
  % removing points in $C$. If $q(0)=p(0)$, then
  %   \[
  %     q(0)
  %       =
  %     p(0)
