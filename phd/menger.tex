...
\begin{document}
  ...
  Obviously $\sigma$-(relatively compact) implies \scish~implies Lindel\"of. We shall see that the non-$\sigma$-(relatively compact) space $\omega_1^\dagger$ is \scish.

  \begin{definition}
    For two functions $f,g$ we say $f$ is \textbf{$\mu$-almost compatible} with $g$ ($f\al\perp_\mu g$) if $|\{x\in\dom(f)\cap\dom(g):f(x)\not=g(x)\}|<\mu$. If $\mu=\omega$ then we say $f,g$ are \textbf{almost compatible} ($f\al\perp g$).
  \end{definition}

  \begin{lemma}\label{kunlem}
    For each $\alpha<\omega_1$, there exist injective functions $f_\alpha:\alpha\to\omega$ such that if $\alpha<\beta$, then \[f_\alpha \al\perp f_\beta\] that is, $f_\alpha$ and $f_\beta\rest\alpha$ agree on all but finitely many ordinals. In addition, the range of each $f_\alpha$ is co-infinite.
  \end{lemma}
  ...
  \begin{proof}
    ...
    Finally, suppose $\gamma$ is the limit of $\alpha_0,\alpha_1,\dots$, and $f_\alpha$ is defined for $\alpha<\gamma$. Let $g_0=f_{\alpha_0}$, and assuming $g_n\al\perp f_{\alpha_n}$ with coinfinite range, define $g_{n+1}:\alpha_{n+1}\to\omega$ so that $g_{n+1}\rest\alpha_n=g_n$ and $g_{n+1}\rest(\alpha_{n+1}\setminus\alpha_n)\al\perp f_{\alpha_{n+1}}$ with coinfinite range. Then $g=\bigcup_{n<\omega} g_n$ is an injective function from $\gamma \to \omega$ and $g \al\perp f_\alpha$ for $\alpha <\gamma$, but the range need not be coinfinite. So let
    \[
      f_\gamma(\beta) = \left\{
      \begin{array}{ll}
      ...
      \right.
    \]
    which frees up $\{g(\alpha_{2n+1}):n<\omega\}$ from the range of $f_\gamma$, and allows $f_\gamma\al\perp f_\alpha$.
  \end{proof}
  ...
  \begin{proof}
    Take the injective funcions $f_\alpha$ from Kunen's lemma such that $f_\alpha \al\perp f_\beta$. For each open cover $\mc V$ of $\oneptlind{\omega_1}$ let $\gamma(\mc V)$ identify the least ordinal such that $[\gamma(\mc V),\omega_1)\cup\{\infty\}$ is in a refinement of $\mc V$. Then $f$ defined by $f(\mc V,0)=[\gamma(\mc V),\omega_1)\cup\{\infty\}$ and $f(\mc{V},n+1) = f_{\gamma(\mc V)}^{-1}(\{n\})$ is easily seen to witness the property.
  \end{proof}
  ...
  \begin{definition} 
    The statement $S(\kappa,\mu,\lambda)$ due to Scheepers is shorthand for the following: there exist injective functions $f_A:A\to\lambda$ for each $A\in[\kappa]^\mu$ such that $f_A\al\perp_\mu f_B$ for all $A,B\in[\kappa]^\mu$.
  \end{definition}
  ...
  \begin{proof}
    Take the injective funcions $f_A:A\to\omega$ witnessing $S(\kappa,\omega,\omega)$. For each cover $\mc V$ of $\oneptlind{\kappa}$ let $A(\mc V)$ define a set such that $\oneptlind{\kappa}\setminus A(\mc V)$ is in a refinement of $\mc V$. Then define $f$ by $f(\mc V,0)=\oneptlind{\kappa}\setminus A(\mc V)$ and $f(\mc{V},n+1) = f_{A(\mc V)}^{-1}(\{n\})$.

    \begin{itemize}
      \item $f(\mc{V},n)$ is $\mc{V}$-compact by definition.
      \item $f(\mc{V},n)\cap f(\mc{V},n+1) = \emptyset$ by definition.
      \item $\bigcup_{n<\omega}f(\mc{V},n)=X$ by definition.
      \item Note that as $f(\mc{U},p+1)=f^{-1}_{A(\mc U)}(\{p\})$, $f(\mc{V},p+1)=f^{-1}_{A(\mc V)}(\{p\})$, and $f_{A(\mc U)}\al\perp f_{A(\mc V)}$, the set
          \[
            \left(\bigcup_{p<\omega} \Big(f(\mc{U},p+1)\setminus f(\mc{V},p+1)\Big) 
            \setminus A(\mc{U})\right)
            \setminus A(\mc{V})
          \] 
        is finite.
  ...
  \begin{definition}
    Let $\pok\subset Fn^2([\kappa]^\omega,\omega)$ be such that each $p_A$ is injective, and give it the partial order $\leq$ defined by $q\leq p$ if and only if:
          \begin{itemize}
            ...
            \item For each $A,B\in\dom(p)$, if $p_A$ and $p_B$ are not defined for some $\alpha\in A\cap B$, but $q_A$ is, then $q_B$ is also defined for $\alpha$ and $q_A(\alpha)=q_B(x)$. That is, 
              \[
  ...
  \begin{theorem}
    $S(\kappa,\omega,\omega)$ is consistent with ZFC for any cardinal $\kappa$.
  \end{theorem}

  \begin{proof}
    If $\kappa\leq\omega_1$, then $S(\kappa,\omega,\omega)$ is a theorem of ZFC - use Lemma \ref{kunlem}.

    Assume $\kappa>\omega_1$; we adapt a forcing argument due to Scheepers. Let $M$ be a countable transitive submodel of ZFC. Consider the c.c.c. poset $\pok$ realized in the model $M$. Let $G$ be a $\pok$-generic filter over $M$.
    ...
    In addition, for $A,B\in [\kappa]^\omega\cap M$, let $p\in G\cap D_A\cap D_B$. For all $q\leq p$ it follows that $\{\alpha\in \dom(q_A)\cap \dom(q_B): q_A(\alpha)\not= q_B(\alpha)\} \subseteq \dom(p_A)\cup\dom(p_B)$. Thus $|\{\alpha\in A\cap B: f_A(\alpha)\not= f_B(\alpha)\}|<\omega$ and $f_A\al\perp f_B$ for $A,B\in [\kappa]^\omega\cap M$, and it's immediate that $f_A\al\perp f_B$ for $A,B\in[\kappa]^\omega$ as well.

    The $f_A$ witness $S(\kappa,\omega,\omega)$ relativized to $M[G]$.
  \end{proof}

  As a result of this forcing for $\kappa>\omega_1$, $CH$ cannot hold in $M[G]$ (Scheepers points out that $\omega_1<\kappa\leq 2^\omega$). It is an open question whether $S(\kappa,\omega,\omega)$ is a theorem of ZFC+CH or ZFC. Nonetheless,

  \begin{corollary}
    $F\kmarkwin{2}\mengame{\oneptlind{\kappa}}$ is consistent with ZFC.
  \end{corollary}
