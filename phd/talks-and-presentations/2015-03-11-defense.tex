...
\begin{frame}
  ...
  In my dissertation I investigate four topological games
  from the literature to find new limited information characterizations.
...
\begin{frame}{
  ...
  such that $f_{\alpha+1}(\beta)<n_F$ for all $\beta\leq\alpha\in F$.
...
Proof that $\pl F\win\menGame{X}$ $\Leftrightarrow$ $\pl F\markwin\menGame{X}$
  ...
  % Since $\sigma(\<\mc U_{s\rest1},\dots,\mc U_{s},\mc U\>)$
  % is finitely coverable by $\mc U$,
  % there is some $F\in[\omega]^{<\omega}$ such that
  % $
  %   \sigma(\<\mc U_{s\rest1},\dots,\mc U_{s},\mc U\>)
  %     \subseteq
  %   \bigcup_{n\in F}U_n
  % $.
  % Since there are only countably many $F\in[\omega]^{<\omega}$,
