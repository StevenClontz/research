...
\begin{document}
  ...
  \begin{proof}
    ...
    form \(\vect x_{A,m}\) where \(A\in\check L\setminus\{\emptyset\}\)
    and \(m\in M\setminus\{0\}\) defined by \(\vect x_{A,m}(l)=m\) for \(l\in A\)
    and \(x_{A,m}(l)=0\) otherwise.

    We claim that the bijection \(\theta(\vect x_{A,m})=\<A,m\>\) is a
    ...
    neighborhood \(Z\) of \(0\) as \(M\) is \(T_2\) or regular.
    ...
    open set \([U,F]\). It follows that \(m\in U(l)\) for all \(l\in C\cap F\),
