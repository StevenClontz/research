...
\begin{document}
  ...
  \begin{proof}
    ...
    is unique, so \(\check L\) is compact but not second countable,
    and therefore nonmetrizable.

    If \(L\) is countable, then note that the union of
    \(\mc S=\{((\leftarrow,l),L]:l\in L\}\) and
    \(\mc T=\{[\emptyset,(\leftarrow,l]):l\in L\}\)
    generates a countable base for \(\check L\). Thus \(\check L\) is
    compact second-countable, and therefore metrizable.
  \end{proof}
