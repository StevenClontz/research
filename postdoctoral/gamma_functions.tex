...
\begin{document}
  ...
  \begin{proof}
    ...
    Consider the basic open neighborhood \([U,F]\) of \(\vect 0\) in
    ...
    \(l\in F\), and thus \(\vect x_{C,m}\in[U,F]\).
    Likewise if we let
    \(
      \vect x_{C,m}
        \subseteq
    ...
    So let \(U(l_0)=W\) and note \(\vect 0\in[U,\{l_0\}]\). For any
    \(\vect x_{C,m}\in[U,\{l_0\}]\), we have two cases.
